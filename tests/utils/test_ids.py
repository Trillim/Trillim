...
class StableIdTests(unittest.TestCase):
    def test_stable_id_is_deterministic(self):
        ...
        self.assertTrue(stable_id("voice_id", b"demo").startswith("voice_id_"))
        self.assertEqual(
            stable_id("voice", Path("/tmp/demo")),
            stable_id("voice", "/tmp/demo"),
        )

    def test_stable_id_validates_arguments(self):
        with self.assertRaisesRegex(ValueError, "prefix"):
            stable_id("-", "demo")
        self.assertTrue(stable_id("voice_id", "demo"))
