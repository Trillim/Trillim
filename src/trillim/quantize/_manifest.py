...
def build_manifest(
    ...
    *,
    ...
    if config.arch_type == ArchitectureType.BONSAI_IMAGE:
        return _build_bonsai_image_manifest(
            model_dir,
            config,
...
def _build_bonsai_image_manifest(
    model_dir: Path,
    config: ModelQuantizeConfig,
    *,
    output_dir: Path,
    ...
    if adapter_dir is not None:
        raise ValueError("Bonsai Image adapter quantization is not supported yet")
    if skip_model:
        raise ValueError("Bonsai Image manifest generation requires model tensors")
    ...
    for section_type, component_dir in (
        (SECTION_IMAGE_TEXT_ENCODER, model_dir / "text_encoder"),
        (SECTION_IMAGE_TRANSFORMER, model_dir / "transformer"),
        (SECTION_IMAGE_VAE, model_dir / "vae"),
    ):
        ...
        component_entries = _append_bonsai_image_component_entries(
            ...
            shard_data_starts=shard_data_starts,
        )
        tensor_entries.extend(component_entries)
        if component_entries:
            sections.append(
                {
                    "type": section_type,
                    "first_tensor_idx": first_tensor_idx,
                    "num_tensors": len(component_entries),
                }
            )
...
def _append_bonsai_image_component_entries(
    ...
    *,
    ...
    shard_files, weight_map = _get_component_sharded_files(component_dir)
    for shard_path in shard_files:
        if shard_path not in shard_idx_map:
            shard_idx_map[shard_path] = len(shard_path_list)
            shard_path_list.append(shard_path)
    ...
    if weight_map:
        ...
        shard_meta = get_tensor_metadata(shard_files[0])
        for item in shard_meta:
            item["file"] = shard_files[0]
        all_tensors_meta.extend(shard_meta)

    entries: list[dict[str, int]] = []
    for item in sorted(all_tensors_meta, key=lambda meta: str(meta["key"])):
        key = str(item["key"])
        file_path = item["file"]
        header = shard_headers[file_path]
        tensor_info = header[key]
...
def _get_component_sharded_files(component_dir: Path) -> tuple[list[Path], dict[str, Path]]:
    try:
        ...
        diffusion_path = component_dir / "diffusion_pytorch_model.safetensors"
